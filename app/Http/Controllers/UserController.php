...
class UserController extends Controller
{
    ...
    public function show($id){
        ...
        // return dd($user_data->utype);

        if(Auth::user()->utype === 'ADM'){
            
            return view('auth.detail', compact('user_data'));

        }elseif(Auth::user()->utype === 'STA'){

            if($user_data->utype === 'ADM'){
    ...
    public function delete_user($id){
        ...
        if($user->is_admin == 0){
            $user->delete();
            return redirect()->back()->with('success', "Deleted acount successfully !");
        }else{
            return redirect()->back()->with('error', "You can't delete admin account");
        }
    }
}
